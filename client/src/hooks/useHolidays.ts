...
export function useHolidays() {
  ...
  const isHoliday = ({date}: DateInputProps) => {
    ...
    const holidayNames = (checkIsHoliday && checkIsHoliday.length) ? checkIsHoliday.map(hd => hd.name): false;
    return holidayNames;
  };

  const isPublicHoliday = ({date}: DateInputProps):false | string[] => {
    const checkIsHoliday = nrwHolidays.isHoliday(new Date(date.toString()));
    const holidayNames = (checkIsHoliday && checkIsHoliday.length) ? checkIsHoliday.filter(hd => hd.type === 'public').map(hd => hd.name) : false;
