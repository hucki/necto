...
export const contract = (p: Patient, c: Company | undefined): jsPDF => {
  ...
  const printFooter = () => {
    ...
    doc.text(
      'Behandlungsvetrag - Seite ' + doc.getCurrentPageInfo().pageNumber,
      13,
      285,
      {}
    );
  ...
  const printParagraph = (text: string, offsetFirstLine: number = 10) => {
    ...
    for (let i = 0; i < text.length; i += lineWidth) {
      ...
      const current =
        remains.length < lineWidth
          ? remains
          : remains.substring(0, lastEmptySpace);
    ...
    res.forEach((value, key) => {
      doc.text(
        value,
        key === 0 ? offsetFirstLine : 13,
        key === 0 ? nextParagraph() : nextLine()
      );
    });
  };
  ...
  doc.text(c?.name2 || '__________________________', centerOffset, nextLine(), {
    align: 'center',
  });
  doc.text(
    c?.street || '__________________________',
    centerOffset,
    nextLine(),
    { align: 'center' }
  );
  ...
  doc.setFont('helvetica', 'italic');
  doc.text('-im Folgenden Logopäd:in genannt-', centerOffset, nextLine(), {
    align: 'center',
  });
  doc.setFont('helvetica', 'normal');
  doc.text('und', 10, nextParagraph());
  doc.text(p.lastName + ', ' + p.firstName, centerOffset, nextParagraph(), {
    align: 'center',
  });
  doc.text(
    'Name der Eltern/Erziehungsberechtigten    ___________________________________',
    centerOffset,
    nextLine(),
    { align: 'center' }
  );
  doc.text(
    'Adresse: ' +
      (p.street || '__________________________') +
      ', ' +
  ...
  doc.text(
    'Versicherung: ' +
      (p.insurance || '__________________________') +
      ', ' +
      'befreit: ' +
      (p.isAddpayFreed ? 'ja' : 'nein'),
    centerOffset,
    nextLine(),
    { align: 'center' }
  );
  doc.setFont('helvetica', 'italic');
  doc.text('-im Folgenden Patient:in genannt-', centerOffset, nextLine(), {
    align: 'center',
  });
  doc.setFont('helvetica', 'normal');
  doc.text(
    'über die Erbringung logopädischer Leistungen.',
    10,
    nextParagraph()
  );
  currentLine += 10;
  const paragraphOneA =
    '1 Die Vertragsparteien schließen einen Vertrag über die Erbringung logopädischer Leistungen. Das Behandlungsverhältnis beginnt mit der Durchführung der Verordnung von';
