...
const ProgressBar = ({ user, userId, userData, events, currentDate }) => {
  ...
  const progress = parseInt(
    (plannedAppointments / userData.appointmentsPerWeek) * 100
  );
  const planningMsg = () => {
    if (progress < 15) return 'Keep planning. You can do it! 😀';
    if (progress < 50) return 'Nearly half there 👍';
    if (progress < 90) return 'Almost there 🚀';
    if (progress < 100) return 'Only a few clicks away 🏁';
    return 'Well done 🏁';
  };
  return (
    <div>
...
const mapStateToProps = (state) => {
  return {
    ...
    userData: state.appointments.teamMembers.filter(
      (member) => member.id === state.userData.currentUserId
    )[0],
